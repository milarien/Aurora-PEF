...
#!/usr/bin/env python3
"""
Aurora CLI Demo (Minimal Invariants) — Patched (Generalized + Optional Adjective)
...
- Explicit assertions: "<Name> has a/an <obj>" where <obj> can be:
    - a single noun: "cat"
    - OR adjective + noun: "black cat"
  (still intentionally narrow; we do NOT try to parse full noun phrases)

- Ambiguous spans: "(her|his|their) <obj>" where <obj> matches the same shape:
    - "her cat"
    - "his black cat"
    - "their loud dog"  (adjective+noun)

- Context admissibility is object-aware: candidate must have explicit "<candidate> has <obj>"

- Resolved interpretation rewrite is object-aware and span-exact.
...
class Fact:
    subject: str
    relation: str
    obj: str  # normalized object string: "cat" or "black cat"
...
class Ambiguity:
    ...
    obj: str         # normalized object: "sister" / "black cat"
    candidates: List[str]
    question: str
...
def normalize_ws(s: str) -> str:
    """Normalize whitespace to single spaces, strip ends."""
    return re.sub(r"\s+", " ", s).strip()


def normalize_obj(s: str) -> str:
    """
    Normalize an object string captured from the utterance.

    - lowercase
    - collapse whitespace
    - keep it as either "noun" or "adj noun" (two tokens max by regex design)
    """
    s = s.lower()
    s = normalize_ws(s)
    return s
...
# Explicit assertion:
#   "<Name> has a/an <noun>"
#   "<Name> has a/an <adj> <noun>"
#
# Capture group 2 = "<noun>" or "<adj> <noun>" (two tokens max)
HAS_OBJ_PATTERN = re.compile(
    r"^\s*([A-Z][a-z]+)\s+has\s+a[n]?\s+([a-z]+(?:\s+[a-z]+)?)\s*\.?\s*$",
    re.IGNORECASE
)

# Ambiguous span:
#   "(her|his|their) <noun>"
#   "(her|his|their) <adj> <noun>"
#
# Capture group 2 = "<noun>" or "<adj> <noun>" (two tokens max)
...
def build_ambiguity(state: State, utterance: str) -> Optional[Ambiguity]:
    ...
    obj = normalize_obj(m.group(2))  # "cat" or "black cat"
    span = f"{slot} {obj}"
...
def normalize_answer(text: str) -> str:
    ...
    tokens = text.split()
    return " ".join(tokens[:6])  # slightly longer to allow "bea's black cat"
